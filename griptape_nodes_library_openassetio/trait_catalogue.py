...
class TraitCatalogue:
    ...
    def trait_ids_for_usage(self, usage: str) -> list[str]:
        """Return a sorted list of all trait IDs with the given usage.

        Includes propertyless traits.

        :param usage: The usage string to filter by (e.g. ``"entity"``, ``"locale"``).

        :returns: Sorted list of trait ID strings.
        """
        return sorted(tid for tid, defn in self._traits.items() if usage in defn.usage)

    def propertied_trait_ids_for_usage(self, usage: str) -> list[str]:
        """Return a sorted list of trait IDs with the given usage that have properties.

        :param usage: The usage string to filter by.

        :returns: Sorted list of trait ID strings.
        """
        return sorted(tid for tid, defn in self._traits.items() if defn.properties and usage in defn.usage)

    def choosable_ids_for_usage(self, usage: str) -> list[str]:
        """Return specification IDs then trait IDs suitable for a picker UI.

        Specifications appear first (sorted), followed by traits (sorted). Includes
        propertyless traits (needed for publishing type selection). Only
        traits/specifications whose usage includes ``usage`` are returned.

        :param usage: The usage string to filter by.

        :returns: Ordered list of choosable ID strings.
        """
        trait_ids = set(self.trait_ids_for_usage(usage))
        spec_ids = sorted(
            sid
            for sid, spec in self._specifications.items()
            if usage in spec.usage and any(tid in trait_ids for tid in spec.trait_ids)
        )
        return spec_ids + sorted(trait_ids)

    def expand_to_trait_ids_for_usage(self, selected_ids: list[str], usage: str) -> list[str]:
        """Expand a mixed list of trait and specification IDs to trait IDs.

        Specifications are replaced by their constituent trait IDs. All traits with the
        given usage are included, even propertyless ones. Duplicates are removed while
        preserving order.
        ...
        """
        valid_ids = set(self.trait_ids_for_usage(usage))
        ...
        # Filter to traits with the requested usage only.
        result = [tid for tid in result if tid in valid_ids]
